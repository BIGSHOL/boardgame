...
- **프로젝트명**: Hanyang: The Foundation (한양: 도읍의 설계자들)
- **기술 스택**: FastAPI + React + Vite + PostgreSQL + Redis
- **특징**: 조선 초기 한양 건설 전략 보드게임 디지털 프로토타입
...
- prompt: |
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../hanyang-phase1-auth`
```
...
| subagent_type | 역할 |
|---------------|------|
| `backend-specialist` | FastAPI, 비즈니스 로직, 게임 로직, WebSocket |
| `frontend-specialist` | React/Vite UI, 상태관리, 게임 보드 렌더링 |
| `database-specialist` | SQLAlchemy, Alembic 마이그레이션, 게임 상태 저장 |
| `test-specialist` | pytest, Vitest, 테스트 작성, 게임 로직 검증 |
...
| 한글 | 영문 | 설명 |
|------|------|------|
| 건물 타일 | Building Tile | 6각형 건물 카드 |
| 청사진 카드 | Blueprint Card | 임무/목표 카드 |
| 워커 | Worker | 견습생/관리 (미플) |
| 자원 | Resource | 목재/석재/기와/먹 |
| 풍수지리 | Feng Shui | 배산임수 보너스 |
| 배산임수 | Baesanimsu | 뒤에 산, 앞에 물 |
